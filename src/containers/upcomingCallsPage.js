...
class UpcomingCallsScreen extends Component {
    state={
        callSchedules:[],
        page: 0,
        loading: false,
    ...
    retrieveCallSchedules = async (isRefresh) => {
        ...
        try{
            ...
            const res = await api.get(`/callSchedule/future/${subjectId}/${currentDate}?timezone=${timeZone}&${serializeQuery(paginationParameters)}`);
            if(res.data) {
...
export default connect(mapStateToProps)(UpcomingCallsScreen);